...
export default class TrashX extends Component {
	constructor(props) {
		super(props);
	...
	componentDidMount() {
		...
		axios.get("https://jkts.herokuapp.com/profil/trashx/"+String(this.state.lokasi))
            .then( (response) => {
            	console.log("4");
                this.setState({ trashx: response.data });
                axios.get("https://jkts.herokuapp.com/data/trashx/"+String(this.state.lokasi))
		            .then( (response) => {
		            	console.log("4");
		            	this.setState({ options:{xaxis:{categories: response.data[0]}} })
		            	this.setState({ series: [{ name: "berat", data: response.data[1] }]
		            				})
		                this.setState({ trashxdata: [response.data[0], response.data[1]] });
		                console.log(this.state.trashxdata[0])
		            })
		            .catch( (error) => {
		                console.log(error);
		           })
		...
		return( 
			<div className="resume">
				<div className="mixed-chart">
					{this.state.trashx !== null ? this.state.trashx.map(this._renderCard) : "loading..."}
				</div>
			    ...
			    <div className="mixed-chart">
			    	{this.state.trashxdata !== null ? this.state.trashxdata.forEach(this._renderCardBaik) : "loading..."}
			    </div>
			</div>
		)	
	}
