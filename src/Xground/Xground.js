...
export default class Xground extends Component {
	constructor(props) {
		super(props);
	...
	componentDidMount() {
		...
		axios.get("https://jkts.herokuapp.com/profil/xground/"+String(this.state.lokasi))
            .then( (response) => {
            	console.log("4");
                this.setState({ xground: response.data });
                axios.get("https://jkts.herokuapp.com/data/xground/"+String(this.state.lokasi))
		            .then( (response) => {
		            	console.log("4");
		...
		return( 
			<div className="resume">
				<div className="mixed-chart">
					{this.state.xground !== null ? this.state.xground.map(this._renderCard) : "loading..."}
				</div>
		    ...
		    <div className="mixed-chart">
			    	{this.state.xgrounddata !== null ? this.state.xgrounddata.forEach(this._renderCardBaik) : "loading..."}
			    </div>
			</div>
		)	
	}
